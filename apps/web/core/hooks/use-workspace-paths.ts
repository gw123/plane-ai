...
export const useWorkspacePaths = () => {
  ...
  const isLegacyAiPath = pathname.includes(`/${workspaceSlug}/pi-chat`);
  const isAgentPath = pathname.includes(`/${workspaceSlug}/agent`);
  const isAiPath = isLegacyAiPath || isAgentPath;
